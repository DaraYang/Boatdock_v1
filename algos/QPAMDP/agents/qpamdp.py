...
class QPAMDPAgent(Agent):
    ...
    def __init__(self, observation_space, action_space, env_name,
                 alpha=0.01,
                 initial_action_learning_episodes=1000,
        ...
        if self.parameter_obs_index is not None:
            ...
            if isinstance(self.parameter_obs_index[0], (list, np.ndarray)):
                ...
                assert isinstance(self.parameter_obs_index[0], np.int64)
        ...
        # name = "incong0"
        experiment_name = f"{env_name}_{timestamp}"
        self.writer = SummaryWriter(log_dir=f'runs/{experiment_name}')
        run = wandb.init(
            project="QPAMDP",
            # config=self.args,
            name = "qpamdp"+env_name,
            dir="../scratch/wandb"
        )
    ...
    def _rollout(self, env, update_actions=False, max_steps=None):
        ...
        while not terminal and not (max_steps is not None and steps > max_steps):
            param = self._parameter_policy(state, act)
            # print (act,param)
            (new_state, time_steps), reward, terminal, info = env.step(self._pad_action(act, param))
        ...
        if self.print_freq > 0 and self._total_episodes % self.print_freq == 0:
            if self.print_freq == 1:
                ...
                print('{0:5s} R:{1:.5f} P(S):{2:.4f}'.format(str(self._total_episodes), sum(returns) / (self._total_episodes),
                                                             succ / len(returns)))
            self.writer.add_scalar('Avg R', sum(returns) / (self._total_episodes), self._total_episodes)
            self.writer.add_scalar('P(S)', succ / len(returns), self._total_episodes)
            self.writer.add_scalar('Total R', sum(rewards), self._total_episodes)
            wandb.log({
                "average reward_rollout": sum(returns) / (self._total_episodes),
                "success_rate_rollout": succ / len(returns),
                "total_reward_rollout": sum(rewards)
            })
