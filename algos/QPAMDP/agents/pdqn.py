...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        self.writer = SummaryWriter(log_dir=f'runs/PDQN/{experiment_name}')
        run = wandb.init(
            project="PDQN",
            name = "pdqn"+env_name,
            # config=self.args,
    ...
    def step(self, state, action, reward, next_state, next_action, terminal, time_steps=1):
        ...
        if self._step >= self.batch_size and self._step >= self.initial_memory_threshold:
            Q_loss, actor_loss = self._optimize_td_loss()
            self.updates += 1
            self.writer.add_scalar("Q_loss", Q_loss,self._step)
            self.writer.add_scalar("actor_loss", actor_loss, self._step)
    ...
    def _optimize_td_loss(self):
        ...
        return loss_Q.item(), Q_loss.item()
