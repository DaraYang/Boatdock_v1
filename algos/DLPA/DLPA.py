...
class Trainer:
    ...
    def evaluate(self, total_timesteps):
        ...
        for epi in range(self.args.eval_eposides):
            ...
            while not terminal:
                t += 1
                state, reward, terminal, info = self.act(action, t, pre_state=state)
        ...
        print("---------------------------------------")
        print(
            f"Timestep {total_timesteps} || Evaluation over {self.args.eval_eposides} episodes_rewards: {np.array(returns).mean():.3f} epioside_steps: {np.array(epioside_steps).mean():.3f} success_rate: {succ / self.args.eval_eposides : .2f}")
        print("---------------------------------------")
        ...
        self.upload_log({"Test_Reward": Test_Reward, 
                         "Test_epioside_step": Test_epioside_step,
                         "Test_success_rate": succ / self.args.eval_eposides})
