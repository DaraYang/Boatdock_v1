...
class Startpoint(Point):
    def __init__(self,name,x_max, x_min, y_max, y_min):
        ...
        img_path = os.path.join(file_dir, "imgs/starticon.png")
        self.icon = 1- cv2.imread(img_path) / 255.0
        self.icon_w = 10
        self.icon_h = 10
...
class Endpoint(Point):
    def __init__(self,name,x_max, x_min, y_max, y_min):
        ...
        file_dir = os.path.dirname(os.path.abspath(__file__))
        img_path = os.path.join(file_dir, "imgs/endicon.png")
        self.icon = 1- cv2.imread(img_path) / 255.0
        self.icon_w = 10
        self.icon_h = 10
        self.icon = cv2.resize(self.icon, (self.icon_h, self.icon_w))
...
class Boatone(Point):
    def __init__(self,name,x_max, x_min, y_max, y_min):
        ...
        file_dir = os.path.dirname(os.path.abspath(__file__))
        img_path = os.path.join(file_dir, "imgs/boat1.png")
        self.icon = 1- cv2.imread(img_path) / 255.0
        self.icon_w = 10
        self.icon_h = 10
        self.icon = cv2.resize(self.icon, (self.icon_h, self.icon_w))
...
class Boattwo(Point):
    def __init__(self,name,x_max, x_min, y_max, y_min):
        ...
        file_dir = os.path.dirname(os.path.abspath(__file__))
        img_path = os.path.join(file_dir, "imgs/boat2.png")
        self.icon = 1- cv2.imread(img_path) / 255.0
        self.icon_w = 10
        self.icon_h = 10
        self.icon = cv2.resize(self.icon, (self.icon_h, self.icon_w))
...
class Boatzero(Point):
    def __init__(self,name,x_max, x_min, y_max, y_min):
        ...
        file_dir = os.path.dirname(os.path.abspath(__file__))
        img_path = os.path.join(file_dir, "imgs/boat0.png")
        self.icon = 1- cv2.imread(img_path) / 255.0
        self.icon_w = 10
        self.icon_h = 10
        self.icon = cv2.resize(self.icon, (self.icon_h, self.icon_w))     
...
class boatEnv(Env):
    def __init__(self,sg=None,choice=None,km=None,testmode=None):
        ...
        self.action_space = spaces.Tuple((
            ...
            spaces.Tuple(  # parameters
                tuple(spaces.Box(np.array([PARAMETERS_MIN[i]]), np.array([PARAMETERS_MAX[i]]), dtype=np.float32) for i in range(num_actions))
            )
        ))  
    ...
    def reset(self):
        ...
        self.start_goal_dist = self.comp_dist(self.startx,self.starty,self.endx,self.endy)
    ...
    def step(self, action):
        ...
        if self.time_limit <= 1:
            ...
            self.truncated = True
            return obs, self.ep_return, done, {"truncated": self.truncated, "success": self.success}
        
        if self.fuel_left <= 1e-6 or self.outofboundary():
            done = True
            self.ep_return = -1
            self.observation_space = self.get_obs() 
            obs = (self.observation_space,1)
        ...
        #     currdist = 1
        # # self.ep_return = self.alpha*(1/currdist) - 2 * ((self.max_fuel-self.fuel_left)/self.max_fuel)
        #     self.epi_return = (self.fuel_left/self.max_fuel) * 5
        ...
        if done == False:
            # self.ep_return = self.alpha*(1/currdist)
            self.ep_return = (self.dist - currdist)/self.start_goal_dist
            # self.ep_return = 0
    ...
    def get_obs(self):
        ...
        return np.concatenate((
            ...
            np.array([self.dist/self.start_goal_dist]))
        )
...
def create_env(choice = None,km = None,testmode = None):
    return boatEnv(choice, km,testmode)