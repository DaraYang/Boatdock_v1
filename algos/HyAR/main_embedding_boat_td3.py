...
def evaluate(env, policy, action_rep, c_rate, episodes=100):
    ...
    for _ in range(episodes):
        ...
        while not terminal:
            ...
            parameter_action = all_parameter_action[0]
...
def run(args):
    ...
    if args.env == "Platform-v0":
        ...
        env = ScaledParameterisedActionWrapper(env)
        env = ScaledStateWrapper(env)
    elif args.env == "boatcong":
        env = boatEnv(choice = 1, testmode = 1)
    elif args.env == "boatincong0":
        env = boatEnv(choice = 0, testmode = 0,km=0)
    elif args.env == "boatincong1":
        env = boatEnv(choice = 0, testmode = 0,km=1)
    ...
    action_parameter_sizes = np.array(
        [env.action_space.spaces[1].spaces[i].shape[0] for i in range(0, discrete_action_dim)]) # changed
    ...
    while total_timesteps < args.max_timesteps:
        ...
        for i in range(max_steps):
            ...
            next_parameter_action = next_all_parameter_action[0]
            ...
            if total_timesteps % args.eval_freq == 0:
                ...
                train_log = {
                'reward': float(total_reward / (t + 1)),
                'reward_100': float(np.array(returns[-100:]).mean()),
                'test_reward_100': float(Test_Reward),
                'test_episode_step_100': float(Test_epioside_step),
                'test_success_rate_100': float(Test_success_rate),
                'step': t
                }
                upload_log(train_log)
    ...
    if not os.path.exists(redir):
        os.mkdir(redir)
    print("redir", redir)
    # title1 = "Reward_td3_boat_embedding_nopre_"
    title2 = "Reward_100_td3_boat_embedding_nopre_"
    title3 = "Test_Reward_100_td3_boat_embedding_nopre_"
    title4 = "Test_epioside_step_100_td3_boat_embedding_nopre_"
    title5 = "Test_success_rate_100_td3_boat_embedding_nopre_"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", default="P-TD3")  # Policy name (P-TD3, DDPG or OurDDPG)
    parser.add_argument("--env", default='boatcong')  # platform goal HFO
    ...
    parser.add_argument("--max_episodes", default=500000, type=int)  # Max time steps to run environment not useful
    parser.add_argument("--max_embedding_episodes", default=1e5, type=int)  # Max time steps to run environment
    parser.add_argument("--max_timesteps", default=5000000, type=float)  # Max time steps to run environment for
