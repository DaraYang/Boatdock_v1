...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        self.action_parameter_sizes = np.array([self.action_space.spaces[1].spaces[i].shape[0] for i in range(0,self.num_actions)]) # changed
        self.action_parameter_size = int(self.action_parameter_sizes.sum())
        self.action_max = torch.from_numpy(np.ones((self.num_actions,))).float().to(device)
        self.action_min = -self.action_max.detach()
        self.action_range = (self.action_max-self.action_min).detach()
        self.action_parameter_max_numpy = np.concatenate([self.action_space.spaces[1].spaces[i].high for i in range(0,self.num_actions)]).ravel() # changed
        self.action_parameter_min_numpy = np.concatenate([self.action_space.spaces[1].spaces[i].low for i in range(0,self.num_actions)]).ravel() # changed
