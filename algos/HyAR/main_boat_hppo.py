...
def evaluate(env, policy,action_parameter_sizes, episodes=100):
    ...
    print("---------------------------------------")
    return np.array(returns[-100:]).mean(), np.array(epioside_steps[-100:]).mean(), suc/episodes
        ...
        for j in range(max_steps):
            ...
            if done or is_to_update:
                ...
                if is_to_update:
                    ...
                    # print("losses",losses)
                    replay_buffer.reset()
                    wandb.log({"loss": losses, "time_steps": total_timesteps})
            ...
            if total_timesteps % args.eval_freq == 0:
                while not done:
                    ...
                    wandb.log({"reward": reward, "time_steps": total_timesteps})

                Test_Reward, Test_epioside_step, test_succ_rate = evaluate(env, policy, action_parameter_sizes, episodes=100)
                
                print('{0:5s} R:{1:.4f} r100:{2:.4f} steps:{3:.4f} success_rate:{4:.2f}'.format(str(total_timesteps), total_reward / (total_timesteps + 1), Test_Reward,
                                                                           Test_epioside_step, test_succ_rate))
                Reward_100.append(Test_Reward)
                Reward.append(total_reward / (total_timesteps + 1))
                epioside_steps_100.append(Test_epioside_step)
                wandb.log({"test_reward_100": Test_Reward, "test_time_steps": Test_epioside_step, "test_success_rate_100": test_succ_rate})
            if done:
                break
...
if __name__ == '__main__':
    ...
    parser.add_argument("--env", default="boatincong0")  # OpenAI gym environment name
    ...
    parser.add_argument("--gpu-no", default='0', type=str)  # Frequency of delayed policy updates
